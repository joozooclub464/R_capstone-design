...
library(psych)
describeBy(ser_inj, ca_yn, mat = T)

#03. 그래프 그리기(박스그래프, 히스토그램)
opar <- par(no.readonly = TRUE)
layout(matrix(c(1,1,2,3),2,2, byrow = TRUE)) #화면분할
boxplot(ser_inj ~ ca_yn)
hist(ser_inj[ca_yn == "무"])
hist(ser_inj[ca_yn == "유"])
par(opar)

#04. 통계분석
# 등분산 검정
var.test(ser_inj ~ ca_yn, data = ca) #0%가 동질하다 ie) 이분산임.

t.test(ser_inj ~ ca_yn,
       data = ca,
