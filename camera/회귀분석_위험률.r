...
plot(num_of_cam ~ total, data = ca)
abline(lm(num_of_cam ~ total, data = ca),
       col = "red",
       lty = 4
)
#03. 상관분석
ca <- ca[c(-88:-89:-90:-91:-92),]
cor(ca, use = "complete.obs", #결측치 
    method = c("pearson")) #상관분석 돌릴 때 방법 3가지중에 피어슨상관분석을 사용한다는 뜻임.
plot(num_of_cam ~ total, data =ca)
abline(lm(num_of_cam ~ total, data = ca), col = "red", lty = 4)
...
#04. 단순회귀분석
regModel <- lm(num_of_cam ~ total, data = ca)
anova(regModel)
summary(regModel)
