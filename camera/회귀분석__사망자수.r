...
plot(num_of_cam ~ dead, data = ca)
abline(lm(num_of_cam ~ dead, data = ca),
       col = "red",
       lty = 4
)
#03. 상관분석
...
plot(num_of_cam ~ dead, data =ca) 
abline(lm(num_of_cam ~ dead, data = ca), col = "red", lty = 4)
...
#04. 단순회귀분석
regModel <- lm(num_of_cam ~ dead, data = ca)
anova(regModel)
summary(regModel)
